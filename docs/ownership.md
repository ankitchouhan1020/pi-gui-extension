# How pi-gui relates to pi

Plain-language model of ownership and live sessions. Diagrams and wire details: [architecture.md](./architecture.md).
...
Standalone (`npm start` / `pi-gui`): hub **creates** sessions in-process.  
`/gui` from TUI: hub **attaches** the TUI’s existing live session (`bound: true`).
